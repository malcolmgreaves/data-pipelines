...
trait SimpleVec {
   ...
   */
  final def zip(v: SimpleVec): Traversable[(Int, Double, Double)] = {
...
object SimpleVec {
   ...
   */
  def dotProduct(v1: SimpleVec, v2: SimpleVec): Double = {
   ...
   */
  def absoluteValue(v: SimpleVec): Double = {
    (0 until v.cardinality).foldLeft(0.0)({
      case (absval, dimension) => absval + Math.abs(v.valueAt(dimension))
    })
  }

  def absElemDiff(v1: SimpleVec, v2: SimpleVec): Double =
    v1.zip(v2).foldLeft(0.0)({
      case (sum, (_, value1, value2)) =>
        sum + Math.abs(value1 - value2)
    })

  def rankFnOrdering[T](f: SimpleVec => Double): ((T, SimpleVec)) => Double =
    (x: (T, SimpleVec)) => f(x._2)
